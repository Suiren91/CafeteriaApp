...
{"total_price","int32_t","integer",4,0,0,1},
...
Orders::Orders(const Row &r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0)
    {
        ...
        {
            totalPrice_=std::make_shared<int32_t>(r["total_price"].as<int32_t>());
        }
    ...
    else
    {
        ...
        if(!r[index].isNull())
        {
            totalPrice_=std::make_shared<int32_t>(r[index].as<int32_t>());
        }
        index = offset + 4;
        if(!r[index].isNull())
        {
...
Orders::Orders(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[3].empty() && pJson.isMember(pMasqueradingVector[3]))
    {
        ...
        {
            totalPrice_=std::make_shared<int32_t>((int32_t)pJson[pMasqueradingVector[3]].asInt64());
        }
    }
...
Orders::Orders(const Json::Value &pJson) noexcept(false)
{
    ...
    if(pJson.isMember("total_price"))
    {
        ...
        {
            totalPrice_=std::make_shared<int32_t>((int32_t)pJson["total_price"].asInt64());
        }
    }
...
const int32_t &Orders::getValueOfTotalPrice() const noexcept
{
    static const int32_t defaultValue = int32_t();
    if(totalPrice_)
        return *totalPrice_;
...
const std::shared_ptr<int32_t> &Orders::getTotalPrice() const noexcept
{
    return totalPrice_;
}
void Orders::setTotalPrice(const int32_t &pTotalPrice) noexcept
{
    totalPrice_ = std::make_shared<int32_t>(pTotalPrice);
    dirtyFlag_[3] = true;
}
...
void Orders::setCreatedAtToNull() noexcept
{
    createdAt_.reset();
    dirtyFlag_[4] = true;
}
...
void Orders::setUpdatedAtToNull() noexcept
{
    updatedAt_.reset();
    dirtyFlag_[5] = true;
}
